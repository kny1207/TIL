...
def signup(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() and profile_form.is_valid():
            ...
            user = form.save()
            profile = profile_form.save(commit=False) # DB에 일단 넣지 않도록 옵션
            profile.user = user
            profile.save()
            # Profile 생성 - 회원가입한 user와 연결시켜주는 것이 user=user
...
def profile_edit(request):
    ...
    if request.method == 'POST':
        ...
        # form에 profile 넣어서 profile 수정 양식 보여주기
